...
from app.models.models import SearchRequest, ChunkModel
from app.core.DatabaseManager import get_session
from app.components.rag_service import RagService
...
async def search_database(
    ...
    dense_method: str = "cosine_distance",
    alpha: float = 0.5,
) -> list[ChunkModel]:
    """
    Full hybrid + rerank search ready for API response.

    Args:
        query: User's search query
        tool_call_attempt: Attempt counter for pagination
        limit: Maximum number of results to return
        dense_method: Vector similarity method to use
        alpha: Weight for combining dense vs sparse results

    Returns:
        List of reranked and formatted chunks
    """

    rag = RagService(db)

    hybrid = rag.hybrid_search(request.query, dense_method, alpha, request.limit)
    chunks = [r["chunk"] for r in hybrid]
    return rag.rerank_and_format(
        chunks, request.query, request.tool_call_attempt, request.limit
    )


# def dense_search(
#     query: str, db: scoped_session, query_method: str = "cosine_distance", n: int = 10
# ) -> list[dict]:
#     """
...
#         query_embedding = (
#             openai.embeddings.create(input=query, model=settings.EMBED_MODEL)
#             .data[0]
#             .embedding
#         )
...
#                 if query_method == "cosine_distance"
#                 else score,
#             }
...
#         return [{"chunk": chunk, "rank": rank} for chunk, rank in results]
#     except Exception as e:
#         print(f"[sparse_search] Search failed: {e}")
#         return []
...
#     max_dense_score = (
#         max(item["similarity_score"] for item in dense_results) if dense_results else 1
#     )
#     for item in dense_results:
#         norm_dense_score = item["similarity_score"] / max_dense_score
#         result_dict[item["chunk"].id] = {
#             "chunk": item["chunk"],
...
#     max_sparse_score = (
#         max(item["rank"] for item in sparse_results) if sparse_results else 1
#     )
...
#                 "chunk": item["chunk"],
#                 "dense_score": 0,
#                 "sparse_score": norm_sparse_score,
#             }
...
#     docs = []
#     id = (
#         tool_call_attempt * limit
#     )  # set starting id based on how many times tool has been called before
...
#             model="rerank-v3.5", query=query, documents=docs, top_n=limit
#         )
...
#             chunk = chunks[index]
#             logging.debug(
#                 f"Relevance score for source {id} with chunk id {chunk.id} and title {chunk.document.title}: {item.relevance_score}"
#             )
