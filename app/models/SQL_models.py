...
class Chunk(Base):
    ...
    )  # creates a link to related document
...
class Conversation(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    session_id = Column(
        UUID(as_uuid=True), index=True, nullable=False, unique=True, default=uuid.uuid4
    )
...
class ChatMessage(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    session_id = Column(
        UUID(as_uuid=True), ForeignKey("conversations.session_id"), index=True
    )
