...
#             return pd.DataFrame(
#                 columns=["ID", "Question", "Answer", "Source File", "Relevant Section"]
#             )
...
#         golden_sources = [
#             self._normalize_path(src.strip()) for src in golden_sources_raw.split(",")
#         ]

#         # Normalize retrieved sources
#         normalized_retrieved = [
#             self._normalize_path(src["url"]) for src in retrieved_sources
#         ]
...
#                     if (g_src, r_src) not in partial_matches and (
#                         r_src,
#                         g_src,
#                     ) not in partial_matches:
#                         partial_matches.append((g_src, r_src))

#         # Calculate metrics
#         precision = (
#             len(exact_matches) / len(normalized_retrieved)
...
#     def evaluate_answer(
#         self, question_id: int, generated_answer: str
#     ) -> Dict[str, Any]:
#         """
...
#             [golden_embedding], [generated_embedding]
#         )[0][0]
...
#         jaccard = (
#             len(golden_tokens.intersection(generated_tokens))
#             / len(golden_tokens.union(generated_tokens))
#             if golden_tokens or generated_tokens
#             else 0
#         )
...
#             sum(a.get("semantic_similarity", 0) for a in answer_results)
#             / len(answer_results)
#             if answer_results
#             else 0
#         )
#         avg_lexical_overlap = (
#             sum(a.get("lexical_overlap", 0) for a in answer_results)
#             / len(answer_results)
#             if answer_results
#             else 0
#         )
...
#     )
#     print(
#         f"Retrieval success rate: {evaluation_results['retrieval_metrics']['success_rate']:.2f}"
#     )
#     print(
#         f"Average retrieval F1 score: {evaluation_results['retrieval_metrics']['avg_f1']:.2f}"
#     )
#     print(
#         f"Average answer semantic similarity: {evaluation_results['answer_metrics']['avg_semantic_similarity']:.2f}"
#     )
